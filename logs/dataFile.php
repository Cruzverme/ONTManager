...
$sel = mysqli_query($conectar,"select count(*) as allcount from log WHERE mac IS NOT NULL");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$sel = mysqli_query($conectar,"select count(*) as allcount from log WHERE mac IS NOT NULL ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$sqlLog = "select * from log WHERE mac IS NOT NULL ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
